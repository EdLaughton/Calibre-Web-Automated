...
def extract_import_manifest_identifiers(manifest: Mapping[str, Any] | None) -> list[str]:
    ...
    - {"identifiers": {"hardcover-id": "123"}}
    - {"identifiers": ["isbn:978...", {"type": "hardcover-id", "value": "123"}]}
    - {
        "provenance": {
            "provider": "hardcover",
            "provider_id": "123",
...
def select_exact_hardcover_result(results: Sequence[Any], existing_identifiers: Mapping[str, Any] | None) -> Any | None:
    ...
    Preference order:
    1. Same Hardcover book id and same Hardcover edition id when available.
    2. Same Hardcover book id and same Hardcover slug when available.
    3. Same Hardcover book id.
    4. First returned result only when no exact Hardcover book id is known.
    """
    if not results:
        return None

    existing_identifiers = existing_identifiers or {}
    hardcover_id = _normalize_text(existing_identifiers.get("hardcover-id"))
    hardcover_edition = _normalize_text(existing_identifiers.get("hardcover-edition"))
    hardcover_slug = _normalize_text(existing_identifiers.get("hardcover-slug"))

    matching_results: list[Any] = []
    for result in results:
        result_identifiers = getattr(result, "identifiers", None)
        if not isinstance(result_identifiers, Mapping):
            result_identifiers = {}

        result_hardcover_id = _normalize_text(result_identifiers.get("hardcover-id"))
        if hardcover_id is None or result_hardcover_id == hardcover_id:
    ...
    if hardcover_edition is not None:
        for result in matching_results:
            ...
            if not isinstance(result_identifiers, Mapping):
                continue
            result_hardcover_edition = _normalize_text(result_identifiers.get("hardcover-edition"))
            if result_hardcover_edition == hardcover_edition:
                return result
    ...
    if hardcover_slug is not None:
        for result in matching_results:
            ...
            if not isinstance(result_identifiers, Mapping):
                continue
            result_hardcover_slug = _normalize_text(result_identifiers.get("hardcover-slug"))
            if result_hardcover_slug == hardcover_slug:
                return result
